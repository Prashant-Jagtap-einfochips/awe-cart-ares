...
if 'variant_root' not in globals():
    variant_root ='out/'
os.environ['VARIANT_ROOT'] = variant_root

from cart.JTest.OpenJADE_code_utility.ojcu import *

from cart.JConfig.cces_gen.create_cces_project import *

import cart.JTest.OJTSI.ConvertRequest
import cart.JTest.OJTSI.ConvertResponse
import cart.JTest.OJTSI.MIPSReport
import cart.JTest.OJTSI.GULPerfMonOJT
import cart.JTest.OJTSI.GULStackMonOJT
import cart.JTest.OJTSI.Ccnt64MonOJT
import cart.JTest.OJTSI.HexagonPktMonOJT
import cart.JTest.OJTSI.PlotExecGraphOJT
import cart.JTest.OJTSI.OJExecLogOJT
import cart.JTest.OJTSI.PassFailReportV2
import cart.JTest.OJTSI.PlotAudioOJT
import cart.JTest.xmlparse.SizeSharcBinary
import cart.JTest.xmlparse.SizeC6678Binary
import cart.JTest.xmlparse.SizeGNUBinary

import cart.JTest.xmlparse.SHARCMemViz
import cart.JTest.postprocess.PingPongStruct

from  cart.JConfig.JTest import *
from  cart.JConfig.JSimulation import *
from  cart.JConfig.SConfig import *
from  cart.JConfig.JFrameworkStru import *
from  cart.JConfig.JSConsUtils import *
from  cart.JAutogen.JAutogen import *
from  cart.JAutogen.JAutogenBafLite import *
from  cart.JAutogen.JAutogenProbe import *

_JTEST_PATH = cart.JTest.__path__._path[0]
JTest_Add('./')
...
AddOption('--arch',
          ...
          help='<processor architecture>: sharc, gul, gxp, hexagon, amd64(linux only), c66, x86(default), x64(dll only), qnx_nto_aarch64')
...
for cfg in [JConfig[test_option] for test_option in sweep_list]:
  ...
  if (ARGUMENTS.get('dump', 0)=='test') or (GetOption('dump_alias') == 'test'):
      ...
      for idx, jconfig in enumerate(JConfig, start=1):
          ...
          test_item += (jconfig['Option']['platform'] if 'platform' in jconfig['Option'] and jconfig['Option']['platform'] else 'None') +', '
          test_item += (jconfig['Option']['runner'] if 'runner' in jconfig['Option'] and jconfig['Option']['runner'] else 'None')
...
# Multi-core Talaria build
        if talaria_sys:
            ...
            if 'codegen' in talaria_model and talaria_model['codegen']:
                ...
                code_gen_files = env.Glob(codegen_folder_prefix+'/*.c', exclude=exclude)
        ...
        if optionLib:
            ...
            for jf_idx, jframework in enumerate(cfg['JFramework']):
                ...
                OpenJADE = env.SConscript(
                    app_root+'/Runtime/Sconscript.py',
            ...
            # Build each JFramework and JModule adapters
                ...
                if ('TalariaModel' in jmodule) and jmodule['TalariaModel']:
                    talaria_model = jmodule['TalariaModel']
                    if 'Path' in talaria_model and os.path.exists(talaria_model['Path']+'/JConfigApp.py'):
                ...
                for jmodule in jframework['JModule']:
                    ...
                    if talaria_sys:
                        ...
                        if not (('TalariaModel' in jmodule) and jmodule['TalariaModel']):
                            ...
                            codegen_folder_prefix = jmodule['CModel']['Path'] if 'CModel' in jmodule else None
#                            continue
                        else:
                            talaria_model = jmodule['TalariaModel']
                            jm_src_path = variant_root+'/slx/'
                            ...
                            if 'Parameters' in jmodule['TalariaModel']:
                                ...
                                jsimcfg = None
                            if 'Path' in talaria_model and os.path.exists(talaria_model['Path']+'/JConfig.py'):
                                env.Depends(jsimcfg, talaria_model['Path']+'/JConfig.py')
                            if 'Path' in talaria_model and os.path.exists(talaria_model['Path']+'/JConfigApp.py'):
                                env.Depends(jsimcfg, talaria_model['Path']+'/JConfigApp.py')
                            if 'Path' in talaria_model and os.path.exists(talaria_model['Path']+'/../JConfigApp.py'):
            ...
            # Build each JModule repo in each JFramework
                    ...
                    if talaria_model:
                        ...
                        if 'codegen' in talaria_model and talaria_model['codegen']:
                            ...
                            exclude = [f'{codegen_folder_prefix}/{x}' for x in talaria_model['exclude']] if ('exclude' in talaria_model and talaria_model['exclude'] is not None) else [];
                            code_gen_files = []
                            code_gen_files.extend(Glob(codegen_folder_prefix+'/*.c', exclude=exclude))
                            code_gen_files.extend(Glob(codegen_folder_prefix+'/*.S', exclude=exclude))

                        talaria_model_path = '#/'+talaria_model['Path'] if 'Path' in talaria_model else None
                    else:
                    ...
                    if talaria_model and 'Path' in talaria_model and talaria_model and os.path.exists(talaria_model['Path']+'/JConfigApp.py'):
            ...
            # Generate File list for OpenJADE Runtime, JFramework, Model and maybe Platform
                ...
                if (optionApp and optionLib):
                    ...
                    JSize = env.MemoryReportBuilder(
                                    ...
                                    source = [JApplication] +
                                    [ _JTEST_PATH + '/xmlparse/ojtsi_size.xsl' ],
                                    arch = optionArch,
