...
{
	if(BAF_Callback_register.post_mask != NULL) {
		BAF_Callback_register.post_mask(exec_mask, arg);
	}
	else {
		//printf("BAF_Callback_post not registered");
	}
...
BAF_Semaphore* BAF_Semaphore_create(uint32_t count){
	if(BAF_Callback_register.semaphore_create != NULL) {
		BAF_Callback_register.semaphore_create(count);
	}
	else {
		//printf("BAF_Callback_sys_process not registered");
	}
...
void BAF_Semaphore_post(BAF_Semaphore* this){
	if(BAF_Callback_register.semaphore_post != NULL) {
		BAF_Callback_register.semaphore_post(this);
	}
	else {
		//printf("BAF_Callback_sys_process not registered");
	}
...
void BAF_Semaphore_pend(BAF_Semaphore* this){
	if(BAF_Callback_register.semaphore_pend != NULL) {
		BAF_Callback_register.semaphore_pend(this);
	}
	else {
		//printf("BAF_Callback_sys_process not registered");
	}
