...
// FIR accelerator debugging support. Define DEBUG_FIR_ACCEL on the build 
// command line to enable FIR accelerator debugging. Note that for profiling
// and production use, DEBUG_FIR_ACCEL should not be defined, ADI_DEBUG
// should NOT be defined for the EMDMA drivers, and compiler optimization
// flags should be SET as stated above. 

#if defined(DEBUG_FIR_ACCEL)
...
#define FIR_ACCEL_EMDMA_STREAM    gEmdmaGetCoreStreamAssignment()


/***  Local Function Declarations  ********************************************/

static bool create_fir_accel_task( firTaskParams_t *params );
static inline void fir_accel_queue_task( firAccelWorkMemory_t *pWork  );
...
/**
 *  fir_accelerator_hw_init()
 *
 *  @brief One-time initialization of hardware used by FIR accelerator
 *
 ...
 */
void fir_accelerator_hw_init( unsigned int *reserved, 
                              unsigned int deviceId,
                              unsigned int coreId )
{
    static bool firAccelDevHwInitialized[GXP_FIR_NUM_DEVS] = { false, false };

    // Validate input parameters
    assert( deviceId < GXP_FIR_NUM_DEVS );
    assert( (coreId == ADI_CORE_SHARC0) || (coreId == ADI_CORE_SHARC1) );

    if ( firAccelDevHwInitialized[deviceId] == false )
    {
        // Initialize the GXP FIR HW Accelerator device. Note, this must be called
        // prior to calling the FIR accelerator device configuration APIs.
        gxpHamResult_t gxpHamResult = gxpHamFirInitAccelDevice( (gxpHamFirDevice_t) deviceId,
                                                                (gxpHamCoreId_t) coreId );
        assert( gxpHamResult == GXPHAM_SUCCESS );  

        // Perform the FIR accelerator HW device initialization only once for the model
        // when the device block instance is first initialized 
        firAccelDevHwInitialized[deviceId] = true;
    }
                           ...
                           unsigned int workMemorySize )
{
    ...
    // Set the device ID of the FIR Accelerator to use for this task
    pWork->firAccelDevId = (gxpHamFirDevice_t) deviceId;
                              ...
                              unsigned int numOut )
{
    ...
    {   
        ...
        for ( int i = 0; i < numInputs; i++ )
        {
            ...
            pEmdmaTcbList[i].pIntStartAddress = (void*) TranslateAddr( (unsigned int) &pInput[0] );
        }
    }
...
unsigned int fir_accelerator_cancel( unsigned int *workMemory )
{
    ...
    gEmdmaResult_t eResult = gEmdmaCancelFastCopy( pWork->emdmaFastReuseCopyReq );
    assert(eResult == GEMDMA_SUCCESS);

    // Cancel any FIR accelerator task processing currently in progress
    gxpHamResult_t aResult = gxpHamFirCancelTask( pWork->hFirTaskHandle );
    assert( aResult == GXPHAM_SUCCESS );
...
static bool create_fir_accel_task( firTaskParams_t *params )
{
    ...
    gxpHamFirTcb_t *ChannelTcb = pWork->firTaskMemory->pChannelList;
    ...
    gxpHamFirAccelTask_t taskParams = 
    {
                                  // Pointer to task memory with unlinked TCB channel list 
        (gxpHamFirTask_t*) pWork->firTaskMemory,
    ...
    gxpHamResult_t result = gxpHamFirCreateAccelTask( pWork->firAccelDevId, &taskParams );
...
static inline void fir_accel_queue_task( firAccelWorkMemory_t *pWork  )
{
    ...
    result = gxpHamFirQueueAccelTask( pWork->hFirTaskHandle );
    assert( result == GXPHAM_SUCCESS );
...
static void emdma_config( firAccelWorkMemory_t *pWork )
{
    gEmdmaResult_t eResult = GEMDMA_SUCCESS;
    ...
    eResult = gEmdmaCreateFastCopy( FIR_ACCEL_EMDMA_STREAM,
...
static inline void emdma_start_transfer( firAccelWorkMemory_t *pWork )
{
    ...
    // to run together. 
    eResult = gEmdmaSubmitFastCopy( pWork->emdmaFastReuseCopyReq );
    assert( eResult == GEMDMA_SUCCESS );
...
static void emdma_cancel( firAccelWorkMemory_t *pWork )
{
    ...
    eResult = gEmdmaCancelFastCopy(pWork->emdmaFastReuseCopyReq);
    assert(eResult == GEMDMA_SUCCESS);    
...
static void setup_variable_work_memory( firAccelWorkMemory_t *pWork, unsigned int *taskMemSize )
{
    ...
    size = (sizeof(*pWork->firTaskMemory) + numOut * sizeof(gxpHamFirTcb_t));
    VERIFY_ITEM_SIZE( (pVarMem - pVarBase + size), varMemSize );
    pWork->firTaskMemory = (gxpHamFirTask_t*) pVarMem; 
    pVarMem += size; 
    *taskMemSize = size;
