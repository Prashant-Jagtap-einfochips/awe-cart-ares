...
// Pool Delay debugging support. Define DEBUG_POOL_DELAY on the build 
// command line to enable Pool Delay debugging. Note that for profiling
// and production use, DEBUG_POOL_DELAY should not be defined, ADI_DEBUG
// should NOT be defined for the EMDMA drivers, and compiler optimization
// flags should be SET as stated above. 

#if defined(DEBUG_POOL_DELAY)
...
#if defined(__ADSPSC594_FAMILY__)
    #define POOL_DELAY_EMDMA_STREAM  gEmdmaGetCoreStreamAssignment()
#elif defined(__ADSPSC569_FAMILY__)
    #define POOL_DELAY_EMDMA_STREAM  GEMDMA_STREAM_1
#endif /* __ADSPSC594_FAMILY__ */
...
static void emdma_config( dmaWorkMemory_t *pWork, const unsigned int *delayChain )
{
    ...
    // copy request.
    eResult = gEmdmaCreateFastCopy(POOL_DELAY_EMDMA_STREAM,
