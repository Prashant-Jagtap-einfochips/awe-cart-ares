 ...
 *  @brief Connects model FIR accelerator code generation with Griffin Lite XP (GLXP)
...
#define FIR_VARIABLE_DATA_SECTION_SIZE( numIn, numOut, numCh, frameSize ) ( \
                ...
                (sizeof(*((firAccelWorkMemory_t*)0)->firTaskMemory) + (numCh) * sizeof(gxpHamFirTcb_t)) + \
...
typedef struct firAccelWorkMemory_s
{
    ...
    // Device ID of GXP FIR Accelerator to use
    gxpHamFirDevice_t firAccelDevId;

    // gxpHam FIR task memory 
    // Buffer located within variable data memory section.
    gxpHamFirTask_t *firTaskMemory;

    // gxpHam FIR task handle
    gxpHamTaskHandle_t hFirTaskHandle;
    ...
    // gxpHam task memory :                     (sizeof(firTaskMemory) + numChannels * sizeof(gxpHamFirTcb_t))
 ...
 */
extern void fir_accelerator_hw_init( unsigned int *reserved, 
