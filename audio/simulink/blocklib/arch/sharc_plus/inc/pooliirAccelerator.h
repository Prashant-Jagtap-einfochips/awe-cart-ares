 ...
 *  @brief Connects model IIR accelerator code generation with Griffin Ultra Lite (GUL)
...
#define IIR_VARIABLE_DATA_SECTION_SIZE( numCh, frameSize ) ( \
                (sizeof(*((iirAccelWorkMemory_t*)0)->iirTaskMemory) + (numCh) * sizeof(gHamIirTcb_t)) + \
...
typedef struct iirAccelWorkMemory_s
{
    ...
    // gHam IIR task memory 
    // Buffer located within variable data memory section.
    gHamIirTask_t *iirTaskMemory;

    // gHam IIR task handle
    gHamTaskHandle_t hIirTaskHandle;
    ...
    //        Name :                                        Size in Bytes
    // -----------------------------------------------------------------------------------
    // gHam task memory :                       (sizeof(iirTaskMemory) + numChannels * sizeof(gHamIirTcb_t))
