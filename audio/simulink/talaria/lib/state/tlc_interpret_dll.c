...
%% INCLUDED BY: InterpretTrigger.tlc
...
#include "rtwtypes.h"
#include "%<LibGetMdlPubHdrBaseName()>.h" // for TSP_COMPAT
...
InterpretEngineIterator requestedStateVar = {-1,-1};
...
%if EXISTS(::TALARIA_ENGINE) && ::TALARIA_ENGINE == "EMPTY"
    return 0;
%else
    return GetNextStateVarName(&listCursor);
%endif
}
...
%if EXISTS(::TALARIA_ENGINE) && ::TALARIA_ENGINE == "EMPTY"
    return 0;
%else
    return GetFirstStateVarName(&listCursor);
%endif
}
...
const char* GetTspsForStateVar(const char* stateVarName)
{
    ...
    if (IsValidInterpretIterator(requestedStateVar))
    {
        ...
        ListTSPsForInterpretation(theInterpretation,outputBuffer, sizeof(outputBuffer));
    }
    return outputBuffer;
}    
...
const int NumTspCompatNumbers = sizeof(TspCompatNumbers) / sizeof(TspCompatNumbers[0]);
