...
if (update_toolchain_info == 1)
    ...
    tc = adsp_sc59x_tc(info.ToolChainName);
    save adsp_sc59x_tc tc;
    if (tc.validate ~= 1)
...
default_cces_dir = 'C:\Analog Devices\CrossCore Embedded Studio 2.10.0';
cces_dir = getenv('CCES_2_10');
if ~exist(cces_dir, 'dir')
    disp('--- Setting CCES 2.10.0 environment variable ---');
    setenv('CCES_2_10', default_cces_dir);
end

% Set the GLXP_LDF_DIR env var used by ert.tlc builds
thisPath = fileparts(mfilename('fullpath'));
setenv('GLXP_LDF_DIR', fullfile(fileparts(thisPath),'build'));
