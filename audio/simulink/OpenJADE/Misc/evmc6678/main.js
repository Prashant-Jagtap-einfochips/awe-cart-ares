        ...
        if (arguments[argCt].match(/^-(.*)/))
        {
            ...
            switch(RegExp.$1)
            {
                ...
                case "-mem_save_log":
                    ...
                    testEnv.memLogFile = arguments[argCt++];
                    break;
                case "msd":
                case "-mem-save-dat":
                case "-mem_save_dat":
...
function run()
{
    ...
    printTrace("Global Default Target Setup...");
    // Call GEL function
    if (debugSession.expression.evaluate("Global_Default_Setup_Silent()") == 1)
    {
       quit(-1);
    } 
    //Thread.currentThread().sleep(60000);//sleep for 1000 ms
    ...
    while (st.hasMoreTokens())
    {
        ...
        if (testEnv.memLogFile != null && !testEnv.asyncRun)
        {
            var nPage = 0;
            var nAddress = debugSession.symbol.getAddress("data_out_binary");
            var sFilename = testEnv.memLogFile;
            var nLength =  debugSession.memory.readData(nPage, debugSession.symbol.getAddress("data_out_length"), 32);
            var nTypeSize = 8;
            var bByteSwap = false;
            
            printTrace("Memory Log Buffer: "+ Long.toHexString(nAddress) +  " Cnt: " + nLength);
            ...
            catch (ex)
            {
                errCode = getErrorCode(ex);
                dssScriptEnv.traceWrite("Error code #" + errCode + ", could not save memory to host!");
                printTrace("Save Log Memory failed with errCode: " + errCode);
                retVal = errCode;
            }
        }
    }
