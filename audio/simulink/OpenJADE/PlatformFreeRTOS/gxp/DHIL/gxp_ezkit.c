...
#define SPORT_4A_SPU          (ADI_SPU0_SPORT4ADDE_ID)
#define SPORT_4B_SPU          (ADI_SPU0_SPORT4BDDE_ID)

#define UART0_SPU_PID         (ADI_SPU0_UART0_ID)
#define UART0_TxDMA_SPU_PID   (ADI_SPU0_UART0TXDDE_ID)
 ...
 *              --------------------------------------------------
 *              J4     J17     J18     J19     J20     J12     J16
 *               O      O       O       O       O       O       O
 *
 *  Input CH                                           1/2     3/4
 *
 * Output CH    1/2    1/2     3/4     5/6     7/8
 *
 *--------------------------------------------------------------------------*/
struct Config_Table
{
...
static int Sport_Init(void)
{
   ...
   CHECK_RESULT(eResult);
   eResult = adi_sport_ConfigMC(hSPORTDev4BRx,7u,7u,0u,true);
   CHECK_RESULT(eResult);
...
static inline int Sport_Enable(void)
{
   /* SPORT return code */
   ADI_SPORT_RESULT    eResult;
...
static int SPU_init(void)
{
   ...
   {
      REPORT_ERROR(SPC "Error: Failed to initialize SPU service\n");
      return FAILED;
   }
   ...
   {
      REPORT_ERROR(SPC "Error: Failed to enable Master secure for SPORT0A\n");
      return FAILED;
   }
   ...
   {
      REPORT_ERROR(SPC "Error: Failed to enable Master secure for SPORT0B\n");
      return FAILED;
   }
   ...
   {
      REPORT_ERROR(SPC "Error: Failed to enable Master secure for UART0\n");
      return (FAILED);
   }
   ...
   {
      REPORT_ERROR(SPC "Error: Failed to enable Master secure for UART0 Tx DMA\n");
      return (FAILED);
   }
    ...
    {
        REPORT_ERROR(SPC "Error: Failed to enable Master secure for MDMA 0 Source\n");
        return (FAILED);
    }
    ...
    {
        REPORT_ERROR(SPC "Error: Failed to enable Master secure for MDMA 0 Destination\n");
        return (FAILED);
    }
    ...
    {
        REPORT_ERROR(SPC "Error: Failed to enable Master secure for MDMA 1 Source\n");
        return (FAILED);
    }
    ...
    {
        REPORT_ERROR(SPC "Error: Failed to enable Master secure for MDMA 1 Destination\n");
        return (FAILED);
    }
#endif /*defined(INTERCORE_XFER_MDMA)*/

    /* Make TRU0 generate secure transactions */
    if( adi_spu_EnableMasterSecure( hSpu, ADI_SPU0_TRU0_ID, true) != ADI_SPU_SUCCESS )
    {
       REPORT_ERROR(SPC "Error: Failed to enable Master secure for TRU0\n");
       return (FAILED);
    }

#if defined(ENABLE_GHAM_MODULE)
    /* Make EMDMA to generate secure transactions */
    if( adi_spu_EnableMasterSecure( hSpu, ADI_SPU0_EMDMA0_ID, true) != ADI_SPU_SUCCESS )
    {
       REPORT_ERROR(SPC "Error: Failed to enable Master secure for EMDMA0\n");
       return (FAILED);
    }
#endif
   return SUCCESS;
...
unsigned char Read_TWI_8bit_Reg(ADI_TWI_HANDLE hTwiDevice, unsigned char Reg_ID)
{
   ...
   {
      REPORT_ERROR(SPC "Error: TWI write failed 0x%08X\n", eResult);
   }
   ...
   if (eResult!=ADI_TWI_SUCCESS)
   {
      REPORT_ERROR(SPC "Error: TWI Read failed 0x%08X\n", eResult);
   }
...
static ADI_TWI_HANDLE Init_TWI(uint32_t *result)
{
   ...
   if (eResult!=ADI_TWI_SUCCESS)
   {
      REPORT_ERROR(SPC "Error: TWI Open failed 0x%08X\n", eResult);
   }

   eResult = adi_twi_SetPrescale(hTwiDevice, PRESCALEVALUE);
   if (eResult!=ADI_TWI_SUCCESS)
   {
      REPORT_ERROR(SPC "Error: TWI Set Prescale failed 0x%08X\n", eResult);
   }

   eResult = adi_twi_SetBitRate(hTwiDevice, BITRATE);
   if (eResult!=ADI_TWI_SUCCESS)
   {
      REPORT_ERROR(SPC "Error: TWI Set Bitrate failed 0x%08X\n", eResult);
   }
   ...
   if (eResult!=ADI_TWI_SUCCESS)
   {
      REPORT_ERROR(SPC "Error: TWI Set Duty cycle failed 0x%08X\n", eResult);
   }

   eResult = adi_twi_SetHardwareAddress(hTwiDevice, TARGETADDR);
   if (eResult!=ADI_TWI_SUCCESS)
   {
      REPORT_ERROR(SPC "Error: TWI Set Hw address failed 0x%08X\n", eResult);
   }
...
static int ADAU_1962_init(ADI_TWI_HANDLE hTwiDevice)
{
   ...
   for (i=0;i<28;i++)
   {
      ...
      {
         REPORT_ERROR(SPC "Error: Configuring ADAU_1962 failed\n");
         return FAILED;
      }
   }
...
static int ADAU_1979_init(ADI_TWI_HANDLE hTwiDevice)
{
   ...
   for (i=0;i<16;i++)
   {
      ...
      {
         REPORT_ERROR(SPC "Error: Configuring ADAU_1979 failed\n");
         return FAILED;
      }
   }
...
void oj_uart_txdma_init(void)
{
   ...
   *pREG_UART0_CLK = (0x80000000) | (sclk0_freq/BAUD_RATE);
   ...
   {
      REPORT_ERROR(SPC "Error: Unable to install UART Tx Handler\n");
      exit(-1);
   }
}
...
void oj_ezkit_init(void)
{
   ...
   {
      REPORT_ERROR(SPC "Error: Power initialization failed\n");
      return;
   }
#endif
   ...
   if( adi_sec_Init() != 0)
   {
      REPORT_ERROR(SPC "Error: SEC initialization failed\n");
      return;
   }
