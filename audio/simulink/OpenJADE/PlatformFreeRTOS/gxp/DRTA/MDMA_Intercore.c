...
#ifdef SPC1
#define SPC_SHARC_L1_SLAVE1_OFFSET SHARC0_L1_SLAVE1_OFFSET
#else
#define SPC_SHARC_L1_SLAVE1_OFFSET SHARC1_L1_SLAVE1_OFFSET
#endif
...
#define pMDMA_RX_STAT_REG       pREG_DMA19_STAT
#define pMDMA_RX_DSCPTR_CUR     pREG_DMA19_DSCPTR_CUR
#define pMDMA_RX_CFG            pREG_DMA19_CFG
#define pMDMA_TX_DSCPTR_CUR     pREG_DMA8_DSCPTR_CUR
#define pMDMA_TX_CFG            pREG_DMA8_CFG
...
#define pMDMA_RX_STAT_REG       pREG_DMA9_STAT
#define pMDMA_RX_DSCPTR_CUR     pREG_DMA9_DSCPTR_CUR
#define pMDMA_RX_CFG            pREG_DMA9_CFG
#define pMDMA_TX_DSCPTR_CUR     pREG_DMA18_DSCPTR_CUR
#define pMDMA_TX_CFG            pREG_DMA18_CFG
...
void MDMA0_RX_Complete_Err_Handler(uint32_t iid, void* handlerArg);
void MDMA1_RX_Complete_Err_Handler(uint32_t iid, void* handlerArg);
void MDMA0_TX_Complete_Err_Handler(uint32_t iid, void* handlerArg);
void MDMA1_TX_Complete_Err_Handler(uint32_t iid, void* handlerArg);
...
volatile unsigned int err_count_mdma0_tx = 0;
volatile unsigned int err_count_mdma1_tx = 0;
volatile unsigned int err_count_mdma0_rx = 0;
volatile unsigned int err_count_mdma1_rx = 0;

volatile unsigned int mdma_rx_isr_count = 0;
...
static inline uint32_t TranslateMdmaAddr( uint32_t MemAddr )
{
    ...
    {
       MemAddr += SPC_SHARC_L1_SLAVE1_OFFSET;
    }
    return MemAddr;
} /* TranslateMdmaAddr() */
...
mdma_result_t MDMA_Init( void )
{
   ...
   mdma_result_t result = MDMA_SUCCESS;
   ...
   int_res |= adi_int_InstallHandler(INTR_SYS_MDMA0_SRC_ERR, MDMA0_TX_Complete_Err_Handler, 0, 1);
   int_res |= adi_int_InstallHandler(INTR_SYS_MDMA1_SRC_ERR, MDMA1_TX_Complete_Err_Handler, 0, 1);
   int_res |= adi_int_InstallHandler(INTR_SYS_MDMA0_DST_ERR, MDMA0_RX_Complete_Err_Handler, 0, 1);
   int_res |= adi_int_InstallHandler(INTR_SYS_MDMA1_DST_ERR, MDMA1_RX_Complete_Err_Handler, 0, 1);
   result = (mdma_result_t) (int_res != ADI_INT_SUCCESS);
#endif
...
void MDMA_Rx_Int_Handler( uint32_t iid, void* handlerArg )
{
   mdma_rx_isr_count++;
   g_mdmaRxDone = 1;
   *pMDMA_RX_STAT_REG = ENUM_DMA_STAT_IRQDONE;
   SYNC_INSTN();
} /* MDMA_Rx_Int_Handler() */
...
static inline void MDMA_Start_SRC_Tx( void )
{
   ...
   *pMDMA_TX_DSCPTR_CUR = (void *) TranslateMdmaAddr( (uint32_t) &MDMA_CORE_TX_TCB[0] );
   *pMDMA_TX_CFG = MDMA_SRC_CONFIG_NO_INT_DSCARRAY_NODE;
   SYNC_INSTN();
...
{
	/* Start in DSCARRAY mode */
   *pMDMA_RX_DSCPTR_CUR = (void *) TranslateMdmaAddr( (uint32_t) &MDMA_CORE_RX_TCB[0] );
   *pMDMA_RX_CFG = MDMA_DST_CONFIG_NO_INT_DSCARRAY_NODE;
   SYNC_INSTN();
} /* MDMA_Start_DST_Rx() */
#endif //#if (NUM_INCOMING_INTERCORE_TRANSFERS > 0)
