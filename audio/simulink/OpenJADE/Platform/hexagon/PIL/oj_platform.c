...
void xmit_control_service_response(FILE *fp, char *service, uint32_t msg_id, uint32_t cnt, uint8_t *buff)
{
   ...
   fprintf(fp, "%s: %8.8lX %8.8lx", service, msg_id, cnt);
   for(idx = 0; idx < cnt; idx++)
   {
...
//*********************************************************************//
// The following functions are empty, not used for standalone build in 
// JTest, but for QuRT OS build in CaPIv2. 
//*********************************************************************//
int qurt_thread_get_id(void)
{
	return 0;
}

long long int qurt_thread_pktcount_get(int a)
{
	return 0;
}

unsigned long long qurt_profile_get_thread_pcycles(void)
{
	return 0;
}