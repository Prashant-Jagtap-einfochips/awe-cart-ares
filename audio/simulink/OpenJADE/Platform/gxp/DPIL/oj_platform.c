...
int platform_fprintf(FILE* fp, char* format, ...)
{
   va_list args;
   int val;
   va_start( args, format );
...
void xmit_control_service_request(FILE * fp, uint32_t cnt, uint32_t *buff)
{
   ...
   platform_fprintf(fp, SPC "REQ:");
   for(idx = 0; idx < cnt; idx++)
   {
      platform_fprintf(fp, " %8.8X", *buff++);
   }
   platform_fprintf(fp, "\n");
...
void xmit_control_service_response(FILE *fp, char *service, uint32_t msg_id, uint32_t cnt, uint8_t *buff)
{
   ...
   platform_fprintf(fp, SPC "%s: %8.8X %8.8x", service, msg_id, cnt);
   for(idx = 0; idx < cnt; idx++)
   {
      platform_fprintf(fp, " %2.2X", *buff++);
   }
   platform_fprintf(fp, "\n");
...
void xmit_control_service_error(FILE *fp, char *service, uint32_t msg_id, int32_t status)
{
   ...
   platform_fprintf(fp, SPC "%s: %8.8X %8.8x\n", service, msg_id, status);
...
#if defined(SPC2)
volatile int32_t FrameSyncInterrupt = 0;
/**
 *  Interrupt Handler for frame sync from SHARC 1
 *
 *  param iid = ID of the interrupt
 *  param handlerArg = pointer to callback function (not used)
 */
static void FrameSyncInterruptHandler(uint32_t iid, void* handlerArg)
{
   FrameSyncInterrupt = 1;
}

static void WaitForFrameSyncInterrupt(void)
{
   FrameSyncInterrupt = 0;

   while (FrameSyncInterrupt != 1)
      asm("nop;");
}
 ...
 */
static void SetupFrameSyncHandling( void )
{
    ADI_SEC_RESULT sec_rtn;
    ADI_INT_STATUS int_rtn;

    /* Setup core 2 TRU event interrupt handler for core 1 frame sync event */
    sec_rtn = adi_sec_EnableEdgeSense( INTR_TRU0_SLV8, true );
    if ( sec_rtn != ADI_SEC_SUCCESS )
    {
       platform_printf(SPC "Error: SetupFrameSyncHandling adi_sec_EnableEdgeSense failed\n");
       return;
    }

    int_rtn = adi_int_InstallHandler( INTR_TRU0_SLV8, FrameSyncInterruptHandler, 0, true );
    if( int_rtn != ADI_INT_SUCCESS )
    {
...
void platform_init(void)
{
    ...
    result = adi_spu_EnableMasterSecure( hSpu, ADI_SPU0_TRU0_ID, true );
    if (result != 0) {
   ...
   {
      platform_printf("Error: Failed to initialize MDMA intercore xfer\n");
      return;
   }
#endif /*defined(INTERCORE_XFER_MDMA)*/
}
...
#if defined(PIL_DCACHEFLUSHINVALIDATE)
