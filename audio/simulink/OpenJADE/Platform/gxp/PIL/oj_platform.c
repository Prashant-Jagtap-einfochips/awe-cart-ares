...
/* LDF L3 uncached memory start and end addresses */
extern "asm" unsigned long ___sdram_uncached_start;
extern "asm" unsigned long ___sdram_uncached_end;

/**
 *  Setup cache controller to remove L3 SDRAM uncached memory 
 *  from cached memory space
 *
 */
void setCacheControllerL3UncachedRange (void) {
   /* Example usage to set an non-cacheable range for OTP in RR5. */
   adiCacheStatus r = adi_cache_set_range((void *)&___sdram_uncached_start, 
                                          (void *)&___sdram_uncached_end, 
                                          adi_cache_rr5, 
                                          adi_cache_noncacheable_range);
   assert(r == adi_cache_success);
}
...
#endif
#endif /*defined(PIL_EZKIT_ONLY)*/
   setCacheControllerL3UncachedRange();
