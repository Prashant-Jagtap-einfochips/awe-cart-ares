...
#include "rtwtypes.h"
#include "BasicAudio_TranslationEngine64.h" // for TOP_COMPAT
...
void InitializeTranslationEngine()
{
    ...
    BasicAudio_TranslationEngine64_initialize();
}
...
const char* GetTuneVarsForTop( const char* topNameIn)
{
    ...
    if (EnsureReturnBuffer())
        return 0;
    theReturnBuffer[0] = 0;
    int retVal = ListTuneVarsForTop(topNameIn, theReturnBuffer,
        theReturnBufferBytes);
...
char* GetTuneVar(const char* name)
{
    ...
    {
        int retVal = PrintTuneVarToBuffer(currentTuneVar, theReturnBuffer,
            theReturnBufferBytes);
        if (retVal > theReturnBufferBytes)
        {
            // Error strings were likely set in the call above
...
const char* GetNextTranslatedTOP()
{
    ...
    while (IsValidTranslateIterator(currentTranslation) && charsDumped == 0 &&
            err == 0)
    {
        ...
        if (currentTopIndex == -1 && err == 0)
        {
            ...
            if ((false == IsValidTranslateIterator(currentTranslation)) && (0 ==
                 sweptAgain))
            {
...
const int NumTopCompatNumbers = sizeof(TopCompatNumbers) / sizeof
    (TopCompatNumbers[0]);
